"""Build (never submit) the benchmark-exact Consensus STEP_2 manifest: the
second of four sequential, chained requests per Consensus donor, built
from STEP_1's REAL, already-retrieved, first-valid responses (see
inference/consensus_benchmark_exact.py's module docstring).

For every donor whose STEP_1 response is locked first-valid, this rebuilds
STEP_1's render (byte-identical prompt-compiler call, same donor/order/
attempt_id=1), reconstructs step1_record by pairing that render with the
REAL retrieved response (this is the one place in the pipeline that reads
an actual model response value -- purely mechanically, to embed as
conversation history for the next turn; never used to decide WHICH donors
get a STEP_2 request, since membership here is exactly "resolved in
STEP_1", an engineering/status fact already established by
score_consensus_exact_stage.py before this script ever runs), then builds
STEP_2's request via build_step2_prompt_render.
...
from inference.consensus_benchmark_exact import CONSENSUS_EXACT_PROTOCOL_ID, build_step1_prompt_render, build_step2_prompt_render, step_record  # noqa: E402
...
OUT_ROOT = PIPELINE_ROOT / "outputs" / "target_production" / "consensus_exact" / "step2"
MANIFEST_FIELDS = [
...
def build_requests(requested_model: str) -> list[BatchRequest]:
    step1_manifest_path = STEP1_DIR / "request_manifest.csv"
    step1_output_path = STEP1_DIR / "retrieved" / "batch_output.jsonl"
    with open(step1_manifest_path, newline="", encoding="utf-8") as f:
        step1_rows = list(csv.DictReader(f))
    if len(step1_rows) != EXPECTED_DONOR_COUNT:
        raise RuntimeError(f"STEP_1 manifest has {len(step1_rows)} rows, expected exactly {EXPECTED_DONOR_COUNT}")
    schema_by_cid = _load_schema_by_cid(STEP1_DIR / "batch_input.jsonl")
    output_by_cid = _load_output_by_cid(step1_output_path)
    ...
    for row in step1_rows:
        ...
        if cid not in output_by_cid:
            raise RuntimeError(f"STEP_1 custom_id {cid} (donor {donor_key}) is not resolved in the retrieved output -- STEP_2 requires every donor to be first-valid in STEP_1")

        profile = _profile_dict(profiles.loc[donor_key])
        render1 = build_step1_prompt_render(profile, donor_key=donor_key, attempt_id=1)
        if render1.request_key != row["request_key"]:
            raise RuntimeError(f"rebuilt STEP_1 request_key for donor {donor_key} does not match the frozen manifest -- refusing to chain from a mismatched render")

        raw_record = output_by_cid[cid]
        validation = validate_response(raw_record, schema_by_cid.get(cid))
        if not validation["valid"]:
            raise RuntimeError(f"STEP_1 response for donor {donor_key} is not schema-valid ({validation['reason']}) -- this contradicts the prior reconciliation report; refusing to chain from an invalid response")

        content = raw_record["response"]["body"]["choices"][0]["message"]["content"]
        response_dict = json.loads(content)
        step1_record = step_record(render1, response_dict, donor_key=donor_key, attempt_id=1)

        render2 = build_step2_prompt_render(profile, donor_key=donor_key, step1_record=step1_record, attempt_id=1)
        key = render2.request_key
        ...
        requests.append(
            BatchRequest(
                ...
                outcome_id="consensus_exact_step2_estimate",
                replicate_id=1,
                requested_model=requested_model,
                prompt_hash=_render_prompt_hash(render2.messages),
                schema_version=schema_hash(render2.response_schema),
                prompt_protocol_id=render2.protocol_id,
                prompt_compiler_version=PROMPT_COMPILER_VERSION,
                seed=seed_from_request_key(key),
                status="pending",
                messages=render2.messages,
                response_schema=render2.response_schema,
                response_key_map=render2.response_key_map or {},
    ...
    if len({r.custom_id for r in requests}) != len(requests):
        raise RuntimeError("duplicate custom_id in Consensus-exact STEP_2 manifest")
    return requests
